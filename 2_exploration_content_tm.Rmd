...
gamma_sect_20 <- read.csv("new_pipeline_sections/data/02_gamma_sections_1880_20.csv")

beta_sect_50 <- read.csv("new_pipeline_sections/data/02_beta_1880_sections_50.csv")
gamma_sect_50 <- read.csv("new_pipeline_sections/data/02_gamma_sections_1880_50.csv")
```
...
meta_1880 %>% 
  ...
  ggplot(aes(x = as.factor(year), y = wordcount, fill = lang_cln)) + geom_col() + 
...
beta_20 %>%
  ...
  facet_wrap(~ topic, scales = "free") +
  scale_y_reordered() + 
  scale_fill_manual(values = rep(cols25(n = 25), times = 2))
```
  
```{r, inlclude = T, echo = F, fig.height=5, fig.width=8, message=FALSE}

gamma_sec_summary <- gamma_sect_20 %>% 
  mutate(title = str_remove_all(document, "\\d+|\\W|_\\w+$")) %>% 
  mutate(year = str_replace_all(document, 
                                "^\\w+(18\\d{2}).*", "\\1"))

gamma_sec_summary <- gamma_sec_summary %>% 
  group_by(title, year, topic) %>% 
  summarise(gamma_avg = mean(gamma))
...
```{r, include = T, echo = F, fig.show='hold', fig.width=8, fig.height=5, message=F}
beta_50 <- beta_sect_50 %>% 
  group_by(topic) %>% 
  slice_max(beta, n = 20)

beta_50 %>% 
  filter(topic %in% c(3, 7, 42, 46)) %>% 
  mutate(term = reorder_within(term, beta, topic)) %>%
  ggplot(aes(beta, term, fill = factor(topic))) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~ topic, scales = "free", nrow = 1) +
  scale_y_reordered() + 
  scale_fill_manual(values = c(wes_palette("Cavalcanti1")[1], wes_palette("Cavalcanti1")[2], 
                               wes_palette("Cavalcanti1")[4], wes_palette("Cavalcanti1")[5]))

gamma_sec_50 <- gamma_sect_50 %>% 
  mutate(title = str_remove_all(document, "\\d+|\\W|_\\w+$")) %>% 
  mutate(year = str_replace_all(document, 
                                "^\\w+(18\\d{2}).*", "\\1"))

gamma_sec_50 <- gamma_sec_50 %>% 
  group_by(title, year, topic) %>% 
  summarise(gamma_avg = mean(gamma))
